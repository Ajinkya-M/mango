    ...
    setCart((prevCart) => {
      ...
      if (existingItem) {
        return prevCart.map((item) =>
          item.variety.id === variety.id ? { ...item, quantity: item.quantity + 1 } : item
        );
      }
        ...
        {step === "phone" && (
          <div className="flex-1 flex flex-col justify-between p-6 sm:p-7 animate-fade-in">
            ...
            <div className="mt-2 text-center flex flex-col items-center justify-center flex-1 min-h-[260px]">
              <MangoLogo className="w-56 h-56 text-amber-600 mb-2" />
                ...
                >
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex gap-3">
                      <div className="w-12 h-12 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center shrink-0">
                        <MangoLogo className="w-10 h-10" varietyColor={m.color} varietyLeafColor={m.leafColor} />
                      </div>
                      <div>
                        <div className="flex items-center gap-1.5">
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 flex justify-between items-center">
                    ...
                    <button
                      ...
                      className="bg-amber-600 hover:bg-amber-700 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg shadow-2xs hover:shadow-xs active:scale-95 transition-all cursor-pointer"
                    >
                      Order Now
                    </button>
                  </div>
            ...
            <div className="flex-1 p-4 overflow-y-auto space-y-4 scrollbar-thin">
              ...
              {mangoVarieties.map((m) => (
                <div
                  ...
                  className="bg-white border border-slate-200 rounded-2xl p-4 shadow-2xs hover:border-amber-500 hover:shadow-xs transition-all duration-200 flex flex-col justify-between relative overflow-hidden cursor-pointer group"
                >
                  <div className="flex justify-between items-start">
                    <div className="flex gap-3">
                      <div className="w-12 h-12 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center shrink-0">
                ...
                <div className="space-y-4">
                  ...
                  {cart.map((item) => (
                    <div key={item.variety.id} className="bg-white border border-slate-200 rounded-2xl p-4 shadow-2xs flex justify-between">
                      <div className="flex gap-3">
                        <div className="w-12 h-12 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center shrink-0">
                          <MangoLogo className="w-10 h-10" varietyColor={item.variety.color} varietyLeafColor={item.variety.leafColor} />
                        </div>
                        <div>
                          <h4 className="font-bold text-xs text-slate-900">{item.variety.name}</h4>
                          <p className="text-[9px] text-slate-400 font-semibold mt-0.5">{item.variety.origin} {item.variety.flag}</p>
                          <p className="text-xs font-bold text-slate-750 mt-1">£{item.variety.pricePerBox.toFixed(2)}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col items-end">
                        <div className="flex items-center gap-2 border border-slate-200 rounded-lg p-1 bg-slate-50">
                          ...
                          <button
                            onClick={() => setCart((prevCart) =>
                              prevCart.map((i) =>
                                i.variety.id === item.variety.id ? { ...i, quantity: i.quantity + 1 } : i
                              )
                            )}
                            className="w-5 h-5 rounded-md bg-white border border-slate-200 flex items-center justify-center text-xs font-bold text-slate-700 hover:bg-slate-100 active:scale-95"
                          >
                        ...
                        <button
                          ...
                          className="mt-2 text-[9px] text-red-600 font-bold hover:underline"
                        >
                          Remove
                        </button>
                      </div>
                  ...
                  <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-2xs space-y-2">
                    {cart.map((item) => (
                      <div key={item.variety.id} className="flex justify-between items-center text-[10px] text-slate-500 font-semibold">
                        <span>{item.variety.name} x{item.quantity}</span>
                        <span>£{(item.variety.pricePerBox * item.quantity).toFixed(2)}</span>
                      </div>
                    ))}
                    ...
                    <div className="flex justify-between items-center text-xs font-extrabold text-slate-900">
                      ...
                      <span>£{cart.reduce((sum, item) => sum + item.variety.pricePerBox * item.quantity, 0).toFixed(2)}</span>
                    </div>
                  </div>
